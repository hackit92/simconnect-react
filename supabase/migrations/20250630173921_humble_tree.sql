/*
  # Fix duplicate categories and inconsistent slugs

  1. Changes
    - Create cleanup function to merge duplicate categories
    - Update product references to use canonical category IDs
    - Remove duplicate categories from database
    - Add unique constraint on slug column
    - Update country_code for products after cleanup

  2. Process
    - Identify all categories with duplicate slugs
    - For each group of duplicates, keep the one with lowest ID as canonical
    - Update all product category_ids arrays to reference canonical IDs
    - Delete duplicate categories
    - Ensure unique constraint prevents future duplicates
*/
